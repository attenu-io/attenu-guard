...
docname: draft-asor-wimse-agent-delegation-chain-00
category: std
ipr: trust200902
area: Security
workgroup: WIMSE
...
Authority:
: The set of permitted actions carried in the "authorization_details" claim
  {{RFC9396}}, together with the constraints in {{constraints}}.
...
Authority is expressed as one or more authorization detail objects {{RFC9396}}.
...
("ceilings") that {{RFC9396}} does not standardize are carried in a "constraints"
member defined here.

## Constraint Vocabulary {#constraints}

The "constraints" member is an object mapping a constraint name to a typed
constraint value. This document defines:

- "max" : a number. The value of the associated quantity MUST NOT exceed it.
  (e.g. {"max_rows": {"max": 5000}}).
...
1. Every authorization detail in C is subsumed by some authorization detail in P
   of the same "type": C's "actions" and "locations" are subsets of P's (with
   prefix and wildcard semantics as defined by the type), and
...
8. Authorize A against DT_n.Authority (scope, actions, locations, and every
   constraint in {{constraints}}). Permit only if A is within it.
...
cacheable and offline-checkable). An online revocation endpoint (RFC 7009) MAY
...
for one resource cannot be replayed at another.
...
{{I-D.niyikiza-oauth-attenuating-agent-tokens}},
{{I-D.coetzee-oauth-spt-txn-tokens}}, {{I-D.klrc-aiagent-auth}}, and the

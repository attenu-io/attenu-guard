...
| **OpenHands** (`openhands-sdk`) 1.44.1 | the `task` tool (`TaskToolSet` → `TaskExecutor` → `TaskManager.start_task`), and `openhands.tools.delegate`'s `DelegateExecutor` (`spawn`) — two independent seams | the same executor hook, filtered on the delegation tool; and `delegate_executor(DelegateExecutor())`, which mints one child per sub-agent id and binds it to ITS conversation (the SDK runs each in a plain `threading.Thread`, which inherits no context) | `ToolDefinition.set_executor(...)` (`openhands/sdk/tool/tool.py`) — `ToolDefinition.__call__` runs the executor and nothing in front of it; `set_executor` returns a `model_copy`, so the tool's class, schema and observation type are untouched. `install(...)` also replaces the tool registry's `_REG` mapping so a tool registered LATER (`register_tool`, a runtime API a sub-agent definition can reach) is guarded too | a scripted `LLM` replaying tool calls | nothing parent-relative: a sub-agent's tools come from its own definition (`.agents/agents/*.md`), never from the parent's list, so a child may hold what its parent does not | 5 |
...
*Fit = how well the framework's official hooks carry an authorization decision (1–5). Twenty entries (LangGraph through A2A: nineteen frameworks and the A2A protocol); `tests/integrations/` holds 33 offline test files covering them, the three recipes below, and the Claude Code and Omnigent examples. The Claude Agent SDK integration was additionally verified live against a real session, and the A2A one over a real HTTP hop.*

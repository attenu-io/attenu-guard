...
*Fit = how well the framework's official hooks carry an authorization decision (1–5). Sixteen frameworks, sixteen offline test suites; the Claude Agent SDK integration was additionally verified live.*
...
live successors beside it. Deliberately not (yet): MCP and A2A (protocols, not frameworks — an MCP server-side
middleware is the natural next adapter), MetaGPT/ChatDev/AutoGPT (research/app-shaped, weak offline story), Letta

...
*Fit = how well the framework's official hooks carry an authorization decision (1–5). Fourteen frameworks, fourteen offline test suites, 249 tests; the Claude Agent SDK integration was additionally verified live.*
...
Claude Agent SDK 8k · Strands 7k. Added later (August 2026), both AutoGen successors:
Microsoft Agent Framework (AutoGen + Semantic Kernel) and AG2 1.0 (the AutoGen fork; a
rewrite whose package is `ag2`, not `autogen`) — AutoGen itself has been in maintenance
since 2026-04, so its adapter now has two live successors beside it. Deliberately not (yet): MCP and A2A (protocols, not
frameworks — an MCP server-side middleware is the natural next adapter),
CAMEL/MetaGPT/ChatDev/AutoGPT (research/app-shaped, weak

...
SCOPE_FOR = scope_map(
    {
        "read_logs": lambda i: ScopeRequest(
            "logs.read", {"rows": int(i["rows"])}
        ),
        "export_findings": lambda i: ScopeRequest(
            "logs.export", {"egress": "any"}
        ),
        "page_oncall": lambda i: ScopeRequest(
            "oncall.page", {"egress": "internal"}
        ),

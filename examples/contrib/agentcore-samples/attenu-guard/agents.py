...
def read_logs(rows: int) -> str:
    ...
    return (
        f"read {rows} lines: 3 spikes in 5xx between 02:10 and 02:40 UTC, "
        "all from the checkout service"
    )
...
def require_guard(entry_agent: Agent, *, guard: DelegationGuard) -> None:
    ...
    if guard.guard_for(entry_agent) is None:
        raise RuntimeError(
            f"attenu-guard holds no permissions for {entry_agent.name!r} — "
...
def denials(guard: DelegationGuard) -> list[dict]:
    ...
    agent_of = {
        entry["node"]: entry["agent"]
        for entry in entries
        if entry["event"] in ("root", "spawn")
    }
    return [
        {

...
an Apache-2.0 library. It ships a Strands hook provider,
`attenu_guard.adapters.strands.DelegationGuard`, registered on each
agent. Strands is unmodified, and nothing is monkeypatched.
...
Versions this was checked against: `strands-agents` 1.53.0,
`bedrock-agentcore` 1.22.0, `attenu-guard` 0.4.1, Python 3.14. The
`bedrock-agentcore-starter-toolkit` provides the `agentcore` CLI and was
not exercised.

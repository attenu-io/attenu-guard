...
| Tool invocation | `DelegationGuard.before_tool_execute` — an `AbstractCapability` registered via `Agent(capabilities=[...])`. `ToolManager._run_execute_hooks` awaits it (`tool_manager.py:459`) before the only path to `toolset.call_tool` (`tool_manager.py:463` -> `1003`), so the tool body provably cannot run on a denial. One registration covers function tools, every toolset, and MCP. |

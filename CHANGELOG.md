...
### Fixed
- **The `a2a` adapter raised out of its own documented contract on an unreadable authority.** `_check_client` and `_check_server` are documented to return `(bool, failures)`, and `_authority_of` to return `Authority | None`. Once `Authority.from_wire` became partial (above), three unguarded call sites meant a bundle carrying a constraint this build cannot read raised a `ValueError` straight out of that contract, handing a caller who correctly handles a failure list an exception instead. All three now report. "Unreadable" is reported distinctly from "absent": both fail closed, but `holds None` for a constraint we could not parse misnames the reason, and the reason is the actionable part. Caught by a code review reading the call sites, because nothing exercised them — the `a2a` extra is not installed in the default dev environment, so the adapter is skipped entirely and a passing suite said nothing about this file. `tests/integrations/test_a2a_unreadable_authority.py` covers it now, confirmed failing before the fix (2 failures, 3 errors) and passing after, alongside the 47 pre-existing a2a tests
- **A Delegation Token carrying authorization data the verifier does not understand verified clean, with that data silently discarded.** Three levels, one defect. (1) `load()` read `authorization_details[0]` and never looked at the rest, so a token carrying `agent_delegation` first and anything after it was accepted and the remaining entries were dropped without a signal. (2) Worse, and missed by the first fix: inside a **single** `agent_delegation` entry the loader cherry-picked `scopes` and `constraints` and discarded every other member — so `actions`, `locations`, `datatypes`, `identifier` and `privileges`, the RFC 9396 §2 common members of *any* authorization detail object, were dropped in silence, as was `critical`, whose entire meaning is "do not ignore me". The draft's token format says "An array of authorization detail objects {{RFC9396}}", so an issuer restricting this way was doing the sanctioned thing. (3) A constraint object carrying two typed values kept one and dropped the other: `{"key":"max_rows","max":100,"min":9999}` became `{"key":"max_rows","max":100}`, byte-identical to one that never carried a floor, though `min` is a first-class type in the draft's constraint vocabulary and the draft allows one typed value per object. Fail-open in exactly one direction, and it is the dangerous one: ignored data that RESTRICTS authority is lost, while data that GRANTS extra authority is harmless because ignoring it leaves the chain more restrictive. Demonstrated end-to-end on the released `valid_chain` vector for every level — the chain verified clean and `permits("crm.read")` returned allowed while the discarded data said otherwise. RFC 8785 canonicalization is not what stops this: a first reproduction was rejected `non_canonical` only because it re-serialized with `json.dumps`, and going through `canonical.dumps` produced a token the verifier accepted. `load()` now refuses, as `malformed`, a token carrying more than one authorization detail, a detail carrying members beyond `type`/`scopes`/`constraints`, or a constraint carrying a member this build does not read; every message names what could not be evaluated. Two `agent_delegation` entries are refused for a second reason: the draft never says which element to take, so taking the first silently nominated a winner the document does not. An unrecognised constraint **type** is deliberately still accepted into the fail-closed `_UnknownCeiling` path the draft requires, rather than becoming a parse error. Refusing is the loosenable direction — if a later revision defines ignorable/critical marking, accepting more stays compatible, whereas starting permissive and tightening later would not. **This is a wire-behaviour change: a token that verified before is now refused.** Nothing we publish is affected — every token of every vector was decoded: maximum detail array length is 1, no detail carries a member outside `{type, scopes, constraints}`, and all 116 constraint objects survive the member check — though that last figure is a vacuous pass and is recorded as one: no vector in either repo carries an `allow`, `deny` or `prefix` constraint, so it never exercised the three ceilings that normalise their values, which is exactly how a first version of the constraint rule looked clean while rejecting conformant tokens (a `one_of` array in any order but the sorted one). Those cases have explicit tests in both ports instead. The identical rules land in the TypeScript port in the same change, and both ports were checked to accept and refuse the same eight cases byte-for-byte through `load()`
...
- **The CrewAI audit-continuity runner (`docs/runs/scripts/crewai_continuity.py`) recorded each model call's message list by reference,** so every call printed the end-of-run list. It stores a copy now, and the run record's two CrewAI blocks are regenerated with a dated correction note

## [0.16.0] - 2026-09-07

### Fixed
- **A `policy` value the format does not define bought an exemption from the containment check.** `policy` is what makes `verify_bundle` SKIP containment on an `allow` — the entry says the chain never authorized the call, so there is nothing to contain — and the value naming that was validated in exactly one place, `_validate_allow`, which runs inside `_execution_binding` and returns early on a `schema_version=1` bundle. So on a v1 chain any non-null value bought it: an `allow` of a scope the node does not hold, stamped `"policy": "totally-made-up"` (or `0`, or `""`), verified clean and reported `containment: True`. Two rules now hold on every version: `policy` may appear only on an `allow` (it was accepted silently on `spawn`, `root` and `outcome` — only `deny` carried the allow-only-field rule), and its value must be one `reasons.Policy` defines. Containment skips only a defined value, so an unknown one is both reported and still measured. New reasons `invalid_policy` and `policy_on_non_allow`, both with README rows; on v2 the existing `invalid_allow` / `invalid_deny` strings are unchanged and no message is emitted twice. A bundle is untrusted input and could carry an unhashable value there, which made the membership test raise `TypeError` out of the verifier rather than report an invalid bundle — that hazard was live on the v2 path too
- **AstrBot: "did the body run?" was answered with per-TOOL state.** `_BodyWitness` is built once per tool, at install time, and the gate set a `ran` flag on it, awaited the body, then read back whatever the last writer left. AstrBot is a chat bot: two conversations, or one model turn with parallel tool calls, invoke the same tool object concurrently on one loop, so the flag belonged to whichever call touched it last — dropping the refusal receipt for a call that was refused, and writing one for a call that DID run. Both are false statements on a tamper-evident ledger. A lock would not have helped; the flag had the wrong lifetime. It is a per-invocation list on a `ContextVar` now, so concurrent calls on one tool share nothing
- **AstrBot: under `strict_single_hook`, a body refused by a nested layer was recorded as an ordinary return.** `_run` closed the outcome before the witness was consulted, so the refusal receipt hit a `DuplicateOutcomeError` that was swallowed, leaving an `outcome/returned` carrying `invoked_params` — an affirmative claim that the body was invoked with those arguments, which is the claim execution binding exists to make true. The one outcome now carries the refusal receipt and no `invoked_params`
- **OpenHands: a delegation the chain refused widened the sub-agent to the parent's authority.** `delegate_executor`'s `spawn` mints a child per sub-agent id after the SDK has created the conversations, and `guard.delegate()` can refuse there (a fanout or depth ceiling, a revoked or expired parent). That was the one delegation site in the adapter with no `except AuthorityError`, so the refusal escaped the loop with the conversations live and unbound — and `active_guard`'s fallback handed them the root. With a fanout of one and ids `["a", "b"]`, `a` was attenuated and `b` silently was not. Each id is decided on its own now, and a refused one is fail-closed: every call from it is denied before any body runs, on the trail. Refusing one id no longer abandons the ids after it
- **A revoked node that was asked again reported `node_finalized` instead of `revoked`.** `Guard.check()` tested finalization before revocation, and a node can be both: an agent finishes its run — whatever records that marks the node complete, as the Google ADK adapter's `after_agent_callback` does — and is revoked afterwards. The refusal was right and the reason was wrong, when `revoked` is what a reader acts on and the closed vocabulary already names it. Revocation is reported first now. Both states still refuse the call, so nothing about WHETHER a call is denied changes, no reason token was added or removed, and every published vector is byte-identical
- **A second user turn in one ADK session was refused with `node_finalized`, in both modes.** `after_agent_callback` marks a node finished when the agent's run returns — correctly, that run did finish — but ADK keeps one session across turns, so on the next turn every call on that agent was refused and the turn produced no work. Two things were wrong: in observe mode the instrument must never change the run it is watching, and in enforce mode a new turn is a new delegation under the same parent, not a call on a dead node. A turn reaching a finalized node now re-spawns that agent under the same parent with the same requested Authority — same `meet`, same ceiling — recorded as an ordinary `spawn`, so the turn runs and the trail shows one node per turn. A **revoked** node is never re-spawned (an agent revoked after a turn is both finalized and revoked, since `after_agent_callback` marks it complete when its run returns — without that check it came back with a live node), nor is one whose parent is revoked or finalized. This is not the resume seam: nothing here re-binds a turn to its original node, which needs a core API and stays a product item
- **The Google ADK adapter did not record a failing context function** — it was missed in the sweep across the other sixteen. Routed through `adapters/_context.py` like the rest: `deny` with `no_authority` and `disposition=unresolved` on the requesting node, then ADK gets this adapter's usual denial dict rather than an exception. A new test asserts every adapter that calls a context callable is on one of the two lists (shared helper, or its own idiom), so the next one cannot be missed silently
...
### Added
- **Two new adapters: OpenHands (`openhands-sdk` 1.44.1) and AstrBot.** `attenu_guard.adapters.openhands` gates every tool through `ToolDefinition.set_executor` and covers both of the SDK's delegation seams (the `task` tool and `DelegateExecutor`); `attenu_guard.adapters.astrbot` installs on the tool-manager registry with a `handler=None` / overriding-`call()` tool, AstrBot's own guarded-tool contract, so the gate is on every invocation path. Nineteen frameworks and the A2A protocol now. New extra `openhands`; AstrBot is an application rather than a distributed package, so it has no extra and runs against a checkout on `PYTHONPATH`, which is how CI tests it. Hooks, versions and what each framework enforces itself: `docs/INTEGRATIONS.md`
...
- Bundle vectors `reject_unknown_policy_value` and `reject_policy_on_spawn`, revision `bundle_vectors_v1.4` (twenty cases): the two rules the accepting `policy` row does not imply, pinned so an implementer cannot make the mistake this release fixed in our own verifier. The first carries an undefined `policy` value on the un-gated allow and MUST reject on both counts — the value is not one the format defines, AND an undefined value buys no containment exemption, so the out-of-authority scope is measured like any other allow. It is the one row in the file with two required failures: either alone would leave the other rule untested. The second puts a DEFINED value on a `spawn`, which a verifier that checks the value but not where it may appear accepts. Both rules are version-independent and both rows are v2, since every bundle in this corpus is a v2 chain; the README says so explicitly and names the v1 path as where reference implementations have gotten it wrong. The eighteen earlier rows are byte-identical, `version` stays `bundle_vectors_v1`, and the corpus description is unchanged

...
- `GuardedToolsetCapability.get_ordering()` feature-detects `CapabilityOrdering.exclusive_execution` and sets it when the installed pydantic-ai has the field. No released version does — checked against 2.31.1, 2.37.0 and 2.39.0; it is on the branch of pydantic/pydantic-ai#8067, where it makes the innermost slot exclusive in every list order and refuses a second capability demanding it. Until it ships, `position="innermost"` is a TIER whose only tiebreak is list order: list `GuardedToolsetCapability` AFTER any other capability that also claims `innermost` and contributes a `call_tool`-overriding wrapper toolset, or that wrapper takes the innermost slot. Both directions are tested. A durability capability (Temporal, DBOS, Prefect) is NOT that case: it swaps LEAF toolsets through `visit_and_replace` rather than wrapping the composed toolset, so the durable wrapper lands inside this guard in either order and the pair composes — until `exclusive_execution` ships, when both set it and pydantic-ai refuses the pair, so an agent runs a durable engine or this capability and not both
- Dual-instrumentation refusal at agent construction covers the new pairs: `GuardedToolsetCapability` beside a `DelegationGuard`, beside a second `GuardedToolsetCapability`, or over a `GuardedToolset` the caller built and passed in `toolsets=[...]`. A capability-contributed `GuardedToolset` never appears in `agent.toolsets`, so both classes now also walk `agent.root_capability`; either `for_agent` refuses, with a message naming both

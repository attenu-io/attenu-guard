...
### Fixed
- **A Delegation Token carrying an authorization detail the verifier does not understand verified clean, with that detail silently discarded.** `load()` read `authorization_details[0]` and never looked at the rest, so a token carrying `agent_delegation` first and anything after it was accepted and the remaining entries were dropped without a signal. Fail-open in exactly one direction, and it is the dangerous one: an ignored entry that RESTRICTS authority is lost, while one that GRANTS extra authority is harmless because ignoring it leaves the chain more restrictive. Demonstrated end-to-end on the released `valid_chain` vector — a trailing detail carrying `deny_scopes: ["crm.read"]` was discarded, the chain verified clean, and `permits("crm.read")` returned allowed, so the verifier permitted the action the issuer's own detail forbade. RFC 8785 canonicalization is not what stops this: a first reproduction was rejected `non_canonical` only because it re-serialized with `json.dumps`, and going through `canonical.dumps` produced a token the verifier accepted. `load()` now refuses a token carrying more than one authorization detail, as `malformed`, naming the entries it could not evaluate. Two `agent_delegation` entries are refused for a second reason: the draft says Authority is expressed by "an" authorization detail of that type and never says which element to take, so taking the first silently nominated a winner the document does not. Refusing is the loosenable direction — if a later revision defines ignorable/critical marking, accepting more stays compatible, whereas starting permissive and tightening later would not. **This is a wire-behaviour change: a token that verified before is now refused.** Nothing we publish is affected — no vector in `src/attenu_guard/vectors/` or `tests/vectors/` carries more than one entry, and the library has never emitted more than one. The identical rule lands in the TypeScript port in the same change, and both ports were checked to accept and refuse the same six orderings byte-for-byte through `load()`

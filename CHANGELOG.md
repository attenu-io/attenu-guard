...
- `GuardedToolsetCapability.get_ordering()` feature-detects `CapabilityOrdering.exclusive_execution` and sets it when the installed pydantic-ai has the field. No released version does — checked against 2.31.1, 2.37.0 and 2.39.0; it is on the branch of pydantic/pydantic-ai#8067, where it makes the innermost slot exclusive in every list order and refuses a second capability demanding it. Until it ships, `position="innermost"` is a TIER whose only tiebreak is list order: list `GuardedToolsetCapability` AFTER any other capability that also claims `innermost` and contributes a `call_tool`-overriding wrapper toolset (a durability capability is the realistic case), or that wrapper takes the innermost slot. Both directions are tested
